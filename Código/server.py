...
################################ MENU OPCIONES ################################################
    ...
    def consultar_estadisticas(self, usuario, socket_usuario):
        ...
        for clave, valor in self.estadisticas_partidas.items():
            if valor['GANADOR'] == str(usuario):
                partidas_ganadas += 1
            
            if valor['JUGADOR1'] == str(usuario):
                contador_partidas += 1
                num_jugadas += int(valor['NUM_JUGADAS'])
        ...
        if contador_partidas != 0:
            ...
            num_jugadas = num_jugadas/contador_partidas
        print(contador_tiempo)
        print(minutos(contador_tiempo))
        mensaje = COD_S_ESTADISTICAS +"||"+ str(contador_partidas)+ "||"+ str(partidas_ganadas)+ "||"+str(minutos(contador_tiempo))+"||"+str(num_jugadas)
    ...
    def cerrar_conexion(self, usuario, socket_usuario): 
        ...
        try:
            ...
            print("ERROR AL INTENTAR REMOVER LA CONEXIÓN A "+ str(usuario))
...
################################ DESARROLLO DE LA PARTIDA ################################################
    ...
    def determinarTipo(self,tipoJugador):
        if tipoJugador== TIPO_JUGADOR_MANUAL:
            ...
            return "Torpe"
        elif tipoJugador==TIPO_JUGADOR_MINMAX:
            return "MinMax"
        elif tipoJugador==TIPO_JUGADOR_MONTECARLO1_ENRIQUE:
            return "MonteCarlo1_Enrique"
        elif tipoJugador==TIPO_JUGADOR_MONTECARLO2_ENRIQUE:
            return "MonteCarlo2_Enrique"
        else:
            return "Indefinido"
