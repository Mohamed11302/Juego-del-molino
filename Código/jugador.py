...
##
from Algoritmo_Enrique.Jugador_Montecarlo_V2 import * 
from Algoritmo_Enrique.Jugador_Montecarlo_V1 import * 
from Algoritmo_Enrique.nodo_montecarlo import * 
from Algoritmo_Enrique.Jugador_Q_Learning import *
global jugador_Enrique_M1
jugador_Enrique_M1 = Jugador_Montecarlo_V1(num_procesos=1, num_iteraciones_montecarlo=ITERACIONES_MONTECARLO)
global jugador_Enrique_M2
jugador_Enrique_M2 = Jugador_Montecarlo_V2(num_procesos=1, num_iteraciones_montecarlo=ITERACIONES_MONTECARLO)
global jugador_Enrique_QLEARNING
jugador_Enrique_QLEARNING = Jugador_Q_Learning()
...
####################### MENU PRINCIPAL DE OPCIONES ##################################################
    ...
    def ConsultarEstadisticas(self):
        ...
        if respuesta[1] == "ERROR":
            ...
            print("#############################################################")
            print("#  NUMERO DE PARTIDAS JUGADAS: "+ str(respuesta[1]))
            print("#  VICTORIAS: "+ str(respuesta[2]))
            print("#  TIEMPO MEDIO: "+str(respuesta[3]))
            print("#  JUGADAS POR PARTIDA: "+str(round(float(respuesta[4]))))
            print("#############################################################")
    ...
    def DeterminarTipoJugador(self):
        ...
        while tipo_jugador == False:
            ...
            print("\t3. Jugador MinMax") 
            print("\t4. Jugador MonteCarlo 1")
            print("\t5. Jugador MonteCarlo 2")
            print("\t6. Jugador MonteCarlo 1 Enrique")  
            print("\t7. Jugador MonteCarlo 2 Enrique")  
            print("\t8. Jugador Q_Learning")
            print("\t9. Jugador Q_Learning Enrique")
            ...
            if tipo == "1":
                ...
                tipo = TIPO_JUGADOR_MONTECARLO1_ENRIQUE
                tipo_jugador = True
            elif tipo == "7": 
                tipo = TIPO_JUGADOR_MONTECARLO2_ENRIQUE
                tipo_jugador = True
            elif tipo == "8":
                ...
                tipo = TIPO_JUGADOR_Q_LEARNING_ENRIQUE
                tipo_jugador = True
        return tipo
...
####################### DESARROLLO DE LA PARTIDA ##################################################
        ...
        if estado_verificado:
            ...
            if self.ComprobarEmpate(contador_jugadas):
                ...
                if tipo_jugador == TIPO_JUGADOR_MANUAL:
                    ...
                    sucesor = self.RealizarMovimientoJugadoresEnrique(estado, jugador_Enrique_M1) #ENRIQUE
                    s = json.dumps(sucesor)
                    free,gamers,chips,turn, move=LeerESTADO_MovimientoRival(s)
                    turn = (turn+1)%2

                elif tipo_jugador == TIPO_JUGADOR_MINMAX: 
                    sucesor = self.RealizarMovimientoJugadorMinMax(estado,PROFUNDIDAD_MINMAX)
                    s = json.dumps(sucesor)
                    free,gamers,chips,turn, move=LeerESTADO_MovimientoRival(s)
                    turn = (turn+1)%2

                elif tipo_jugador == TIPO_JUGADOR_MONTECARLO2_ENRIQUE:
                    sucesor = self.RealizarMovimientoJugadoresEnrique(estado, jugador_Enrique_M2)
                    s = json.dumps(sucesor)
                    free,gamers,chips,turn, move=LeerESTADO_MovimientoRival(s)
                    turn = (turn+1)%2
                
                elif tipo_jugador == TIPO_JUGADOR_Q_LEARNING_ENRIQUE:
                    sucesor = self.RealizarMovimientoJugadoresEnrique(estado, jugador_Enrique_QLEARNING)
                    s = json.dumps(sucesor)
                    free,gamers,chips,turn, move=LeerESTADO_MovimientoRival(s)
                    turn = (turn+1)%2
    ...
    def RealizarMovimientoJugadorMinMax(self, estado, profundidad):
        sucesores=generarSucesores(estado)
        arbol = minmax.arbolMin(profundidad, sucesores)
        return arbol.devolverJugada()

    def RealizarMovimientoJugadoresEnrique(self, estado, jugador_Enrique): #ENRIQUE
        try:
            e = json.loads(estado)  
            if e['STATE'] == None:
                e = None
            ...
            e=estado
        if e == None:
            sucesor = jugador_Enrique.genera_movimiento(None)
        else:
            sucesor = jugador_Enrique.genera_movimiento(e)
        return sucesor[1]
